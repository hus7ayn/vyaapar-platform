...
export function ShopSwitcher() {
  ...
  // Matches BranchScopeGuard on the backend — only BUSINESS_MANAGE holders can
  // actually switch branches; everyone else is silently pinned to their own
  // branchId server-side, so showing them an interactive picker is misleading.
  const canSwitchShops = has(Permission.BUSINESS_MANAGE);
  const canManageShops = has(Permission.BRANCH_MANAGE);
    ...
    <div className="px-3 py-2 border-b bg-[hsl(348,30%,97%)]">
      <div className="flex items-center gap-2">
        ...
        {canSwitchShops ? (
          ...
          <div className="flex-1 min-w-0 truncate px-2.5 py-1.5 text-sm font-medium">
